...
import com.example.myapplication.adapter.CahrtVPAdapter;
import com.example.myapplication.data_backage.DBManager;
import com.example.myapplication.utils.CalendarDialog;
import com.example.myapplication.frag_chart.IncomeChartFragment;
import com.example.myapplication.frag_chart.OutcomeChartFragment;
...
public class MonthChartActivity extends AppCompatActivity {
    ...
    private CahrtVPAdapter chartVPAdapter;
    ...
    private void initFrag() {
        ...
        //使用适配器
        chartVPAdapter = new CahrtVPAdapter(getSupportFragmentManager(), chartFragList);
    ...
    private void initStatistics(int year, int month) {
        float inMoneyOneMonth = DBManager.getSummonth(year, month, 1);  //收入总钱数
        float outMoneyOneMonth = DBManager.getSummonth(year, month, 0); //支出总钱数
    ...
    private void setButtonStyle(int kind){
        if (kind == 0) {
            outBtn.setBackgroundResource(R.drawable.draw_btn_bg_);
            outBtn.setTextColor(Color.WHITE);
            inBtn.setBackgroundResource(R.drawable.dialog_btn_bg);
            inBtn.setTextColor(Color.WHITE);
        }else if (kind == 1){
            inBtn.setBackgroundResource(R.drawable.draw_btn_bg_);
            inBtn.setTextColor(Color.WHITE);
